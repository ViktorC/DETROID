...
public class SizeEstimator {
  ...
  /**
   * Returns whether all instances of the specified class must have the same memory requirements. This can be only guaranteed if all the non
   * static fields of the class are primitives.
   *
   * @param clazz The class.
  ...
  /**
   * Estimates the size of an object including the sizes of all objects referenced in the object graph. It traverses an acyclic graph by not
   * visiting nodes more than once and thus ensuring that even if there are multiple references pointing to the same object, its size is
   * added to the graph's total size only once.
   *
   * @param o The object.
  ...
  /**
   * Statically estimates the size of an instance of the class. The estimate is only reliable if {@link #isInstanceSizeConstant(Class)
   * isInstanceSizeConstant} returns true, i.e. all the non-static fields of the class and its ancestors are primitives. The non-primitive
   * non-static fields are assumed to be null pointers.
   *
   * @param clazz The class.
