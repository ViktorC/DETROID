...
public class Cache<T extends Cache.Entry<T>> implements Iterable<T> {
	 ...
	 */
	public void clear() {
		for (T entry : t1) entry.empty();
		for (T entry : t2) entry.empty();
		size = 0;
	}
	@Override
	...
	/**
	 * An interface for hash table entries that extends the {@link java.lang.Comparable} interface and 
	 * defines methods required for the management of entries in the hash table.
	 * 
	 * @author Viktor
	 *
	...
	/**
	 * A factory interface for creating table entries. It serves the purpose of initializing all the entries of the 
	 * hash table so as to pre-allocate the required heap memory and take advantage of spatial locality.
	 * 
	 * @author Viktor
	 *
	 * @param <T> A hash table entry type that implements the {@link net.viktorc.detroid.framework.util.Cache.Entry} interface.
