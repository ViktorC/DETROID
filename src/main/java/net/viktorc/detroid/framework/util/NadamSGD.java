...
public abstract class NadamSGD<D, L> {
   ...
   * @param minValues The minimum allowed values for the features. Each element corresponds to the element at the same index in the features
   * array. If the length of the array is greater than that of the features array, the extra elements will be ignored. If the length of the
   * array is smaller than that off the features array, the array will be extended by elements of the greatest negative double value to
   * match the length of the features array. If it is null, an array of elements of the greatest negative double value will be used. Each
   ...
   * corresponding feature will be ignored.
   * @param maxValues The maximum allowed values for the features. Each element corresponds to the element at the same index in the features
   * array. If the length of the array is greater than that of the features array, the extra elements will be ignored. If the length of the
   * array is smaller than that off the features array, the array will be extended by elements of the greatest positive double value to
   * match the length of the features array. If it is null, an array of elements of the greatest positive double value will be used. Each
   ...
   * cost function treats the features as integers, a value of less than 1 or any non-integer value whatsoever would make no sense), thus it
   ...
   * @param momentumDecayRate A constant that determines the base decay rate of the accumulated Nesterov momentum. If it is null, a default
   * value of 0.99 is used. The lower this value, the faster the decay. It is not recommended to change this value. However, if it is
   * changed, the new value has to be within the range of 0 (inclusive) and 1 (inclusive).
   * @param normDecayRate A constant that determines the base decay rate of the accumulated gradient squares. If it is null, a default value
   * of 0.999 is used. The lower this value, the faster the decay. It is not recommended to change this value. However, if it is changed,
   * the new value has to be within the range of 0 (inclusive) and 1 (inclusive).
   * @param annealingExponent A constant that determines the momentum's annealing schedule. If it is null, it defaults to 4e-3. It is not
   ...
   * @throws IllegalArgumentException If features is null or its length is 0. If the decay rate is greater than 1 or smaller than 0. If an
   * element in minValues is greater than the respective element in maxValues.
   */
  protected NadamSGD(double[] features, double[] minValues, double[] maxValues, Double h, Double baseLearningRate,
   ...
   * approximate the derivative of the cost function for the training data sample with respect to the parameter at index i in the features
   * array.
   *
  ...
  /**
   * Calculates the costs associated with the given feature set for the specified data sample. The better the system performs, the lower the
   * costs should be. Ideally, the cost function is differentiable, smooth, and convex, but these are not requirements.
   *
