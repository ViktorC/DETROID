...
public class GaviotaTableBaseJNI extends EndGameTableBase {
   ...
   * @param dtm Whether the result should include DTM information as well; which is more expensive.
   * @param soft Whether only the cache should be probed.
   * @return If the position is found, a {@link net.viktorc.detroid.framework.engine.EndGameTableBase.DTM} or {@link
   * net.viktorc.detroid.framework.engine.EndGameTableBase.WDL} instance depending on the <code>dtm</code> argument; else null.
   */
   ...
   *
   * @param verbose Whether information regarding the state of the library after initialization should be returned.
   ...
   * @param wdlFraction The fraction of the cache (of a total of 128) that should be reserved for WDL information.
   ...
   *
   * @param intStats A long array of length 14. The elements of the array after the execution of the method contain the following
   * statistics:<br>
   ...
   * @param fpStats A double array of length 3. The elements of the array after the execution of the method contain the following
   * statistics:<br>
  ...
  /**
   * Probes the cache for both WDL and DTM information for the given position. If the cache lookup fails, it proceeds to search the files.
   *
   * @param sideToMove An integer denoting the side to move. <code>0</code> stands for white, <code>1</code> stands for black.
   * @param enPassantSqr The index of the destination square of a potential en passant or {@link #NO_SQUARE} if none.
   * @param catlingRights An integer representing the castling rights in the given position. It is the union of the appropriate castles
   * represented by {@link #NO_CASTLE}, {@link #WSHORT}, {@link #WLONG}, {@link #BSHORT}, and {@link #BLONG}.
   * @param wSqrs An array of the square indices of all the white pieces and a final entry containing {@link #NO_SQUARE}.
   * @param bSqrs An array of the square indices of all the black pieces and a final entry containing {@link #NO_SQUARE}.
   * @param wPcs An array of the pieces as defined by the constants {@link #PAWN}, {@link #KNIGHT}, {@link #BISHOP}, {@link #ROOK}, {@link
   * #QUEEN}, and {@link #KING}. Each element of the array denotes the piece on the square the element at the same index in the
   * <code>wSqrs</code> array represents. The array also contains a final element denoting the end of the array with the value {@link
   * #NO_PIECE}.
   * @param bPcs An array of the pieces as defined by the constants {@link #PAWN}, {@link #KNIGHT}, {@link #BISHOP}, {@link #ROOK}, {@link
   * #QUEEN}, and {@link #KING}. Each element of the array denotes the piece on the square the element at the same index in the
   * <code>bSqrs</code> array represents. The array also contains a final element denoting the end of the array with the value {@link
   * #NO_PIECE}.
   * @return An two-element integer array or <code>null</code> if the position was not found. The first element of the array contains the
   * WDL information while the second one contains the distance to mate.
   */
  public native int[] probe(int sideToMove, int enPassantSqr, int catlingRights, int[] wSqrs, int[] bSqrs, char[] wPcs, char[] bPcs);

  /**
   * Probes the cache for both WDL and DTM information for the given position. If the cache lookup fails, it returns <code>null</code>.
   *
   * @param sideToMove An integer denoting the side to move. <code>0</code> stands for white, <code>1</code> stands for black.
   * @param enPassantSqr The index of the destination square of a potential en passant or {@link #NO_SQUARE} if none.
   * @param catlingRights An integer representing the castling rights in the given position. It is the union of the appropriate castles
   ...
   * <code>bSqrs</code> array represents. The array also contains a final element denoting the end of the array with the value {@link
   * #NO_PIECE}.
   * @return An two-element integer array or null if the position was not found. The first element of the array contains the WDL information
   * while the second one contains the distance to mate.
   */
  public native int[] probeSoft(int sideToMove, int enPassantSqr, int catlingRights, int[] wSqrs, int[] bSqrs, char[] wPcs, char[] bPcs);

  /**
   * Probes the cache for WDL information for the given position. If the cache lookup fails, it proceeds to search the files.
   *
   * @param sideToMove An integer denoting the side to move. <code>0</code> stands for white, <code>1</code> stands for black.
   * @param enPassantSqr The index of the destination square of a potential en passant or {@link #NO_SQUARE} if none.
   * @param catlingRights An integer representing the castling rights in the given position. It is the union of the appropriate castles
   ...
   * <code>bSqrs</code> array represents. The array also contains a final element denoting the end of the array with the value {@link
   * #NO_PIECE}.
   * @return An integer array containing WDL information as defined by the constants {@link #WMATE}, {@link #BMATE}, {@link #DRAW}, {@link
   * #FORBID}, and {@link #UNKNOWN}.
   */
  public native int probeWDL(int sideToMove, int enPassantSqr, int catlingRights, int[] wSqrs, int[] bSqrs, char[] wPcs, char[] bPcs);

  /**
   * Probes the cache for WDL information for the given position. If the cache lookup fails, it returns {@link #UNKNOWN}.
   *
   * @param sideToMove An integer denoting the side to move. <code>0</code> stands for white, <code>1</code> stands for black.
   * @param enPassantSqr The index of the destination square of a potential en passant or {@link #NO_SQUARE} if none.
   * @param catlingRights An integer representing the castling rights in the given position. It is the union of the appropriate castles
   ...
   * #FORBID}, and {@link #UNKNOWN}.
   */
  public native int probeSoftWDL(int sideToMove, int enPassantSqr, int catlingRights, int[] wSqrs, int[] bSqrs, char[] wPcs, char[] bPcs);
  ...
  public void init(String path, long cacheSize, Object... args) {
    ...
    String absPath = Arrays.stream(path.split(";")).collect(StringBuilder::new,
        (s, p) -> s.append(";").append(p), StringBuilder::append).toString();
    if (!hasBeenInit) {
