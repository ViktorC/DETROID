...
/**
 * A class for evaluating chess positions. It uses an evaluation hash table ato improve performance. It also offers a 
...
public class Evaluator {
	...
	public short materialValueByPieceInd(int pieceInd) {
		if (pieceInd == Piece.W_KING.ordinal()) return params.kingPhaseWeight;
		else if (pieceInd == Piece.W_QUEEN.ordinal()) return params.queenValue;
		else if (pieceInd == Piece.W_ROOK.ordinal()) return params.rookValue;
		else if (pieceInd == Piece.W_BISHOP.ordinal()) return params.bishopValue;
		else if (pieceInd == Piece.W_KNIGHT.ordinal()) return params.knightValue;
		else if (pieceInd == Piece.W_PAWN.ordinal()) return params.pawnValue;
		else if (pieceInd == Piece.B_KING.ordinal()) return params.kingValue;
		else if (pieceInd == Piece.B_QUEEN.ordinal()) return params.queenValue;
		else if (pieceInd == Piece.B_ROOK.ordinal()) return params.rookValue;
		else if (pieceInd == Piece.B_BISHOP.ordinal()) return params.bishopValue;
		else if (pieceInd == Piece.B_KNIGHT.ordinal()) return params.knightValue;
		else if (pieceInd == Piece.B_PAWN.ordinal()) return params.pawnValue;
		else return 0;
	}
	/**
	...
	short SEE(Position pos, Move move) {
		...
		dB = MoveSetBase.values()[move.getTo()];
		do {
			i++;
	 ...
	 * @return The phase estimate on a scale from 0 to 255.
	 */
	...
	/**
	 * A static evaluation of the chess position from the color to move's point of view. It considers material
	 * imbalance, coverage, pawn structure, queen-king tropism, mobility, immediate captures, etc. It assumes that the
	 * position is not a check.
	 * 
	...
	int score(Position pos, byte hashGen, int alpha, int beta, ETEntry entry) {
		short score = 0;
		boolean gotBaseScore = false;
		// Probe evaluation hash table.
		ETEntry eE;
		if (eT != null && (eE = eT.get(pos.getKey())) != null) {
			short entryScore = eE.getScore();
			boolean entryExact = eE.isExact();
			if (eE.hashKey() == pos.getKey()) {
				...
				score = (short) (pos.isWhitesTurn() ? entryScore : -entryScore);
				gotBaseScore = true;
			}
		}
		if (!gotBaseScore) {
			...
			if (numOfWhiteBishops >= 2 &&
					Diagonal.getBySquareIndex(BitOperations.indexOfLSBit(pos.getWhiteBishops())).ordinal()%2 !=
					Diagonal.getBySquareIndex(BitOperations.indexOfLSBit(BitOperations.resetLSBit(pos.getWhiteBishops()))).ordinal()%2)
				score += params.bishopPairAdvantage;
			if (numOfBlackBishops >= 2 &&
					Diagonal.getBySquareIndex(BitOperations.indexOfLSBit(pos.getBlackBishops())).ordinal()%2 !=
					Diagonal.getBySquareIndex(BitOperations.indexOfLSBit(BitOperations.resetLSBit(pos.getBlackBishops()))).ordinal()%2)
				score -= params.bishopPairAdvantage;
			// Tempo advantage
			short tempScore;
			if (pos.isWhitesTurn()) {
			...
			if (eT != null && (tempScore <= alpha - params.lazyEvalMar || tempScore >= beta + params.lazyEvalMar)) {
				entry.set(pos.getKey(), tempScore, false, hashGen);
				entry.setupKey();
				eT.put(entry);
				return tempScore;
			}
		...
		// Stopped pawns.
		score += params.stoppedPawnWeight*(BitOperations.hammingWeight((pos.getBlackPawns() >>> 8) &
				(pos.getAllWhiteOccupied()^pos.getWhitePawns())) -
				BitOperations.hammingWeight((pos.getWhitePawns() << 8) &
						(pos.getAllBlackOccupied()^pos.getBlackPawns())));
		if (eT != null) {
			entry.set(pos.getKey(), score, true, hashGen);
			entry.setupKey();
			eT.put(entry);
		}
		return score;
