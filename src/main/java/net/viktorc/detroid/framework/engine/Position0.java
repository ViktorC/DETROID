...
/**
 * A bitboard board based chess position class whose instances hold information amongst others on the current board position, on all the previous moves
...
class Position0 {
	...
	// Bit board unions maintained for the faster processing of positions.
	...
	/**
	 * A complimentary board data-structure to the bitboard boards to efficiently detect pieces on specific squares.
	 */
	byte[] offsetBoard;
	 ...
	 */
	ArrayDeque<PositionStateRecord> unmakeRegisterHistory;
	...
	/**
	 * A Zobrist key that is fairly close to a unique representation of the state of the Position0 instance in one 64 bitboard number.
	 */
	long key;
	...
	/**
	 * It parses a FEN-String and initializes a Position0 instance based on it.
	 ...
	 */
	static Position0 parse(String fen) throws ChessParseException {
		Position0 pos = new Position0();
		...
		if (fenFields.length == 6) {
			...
			} catch (NumberFormatException e) { throw new ChessParseException("The fifty-move rule clock field of the " +
		...
		for (int i = 7; i >= 0; i--) {
			...
			for (int j = 0; j < rank.length(); j++) {
				...
				if (pieceNum >= 0 && pieceNum <= 8)
					...
					switch (piece) {
						case 'K':
							pos.whiteKing = Square.getByIndex(index).bitboard;
						break;
						case 'Q':
							pos.whiteQueens	|= Square.getByIndex(index).bitboard;
						break;
						case 'R':
							pos.whiteRooks |= Square.getByIndex(index).bitboard;
						break;
						case 'B':
							pos.whiteBishops |= Square.getByIndex(index).bitboard;
						break;
						case 'N':
							pos.whiteKnights |= Square.getByIndex(index).bitboard;
						break;
						case 'P':
							pos.whitePawns |= Square.getByIndex(index).bitboard;
						break;
						case 'k':
							pos.blackKing = Square.getByIndex(index).bitboard;
						break;
						case 'q':
							pos.blackQueens |= Square.getByIndex(index).bitboard;
						break;
						case 'r':
							pos.blackRooks |= Square.getByIndex(index).bitboard;
						break;
						case 'b':
							pos.blackBishops |= Square.getByIndex(index).bitboard;
						break;
						case 'n':
							pos.blackKnights |= Square.getByIndex(index).bitboard;
						break;
						case 'p':
							pos.blackPawns |= Square.getByIndex(index).bitboard;
					}
					index++;
				}
			}
	...
	/**
	 * Initializes a default, empty Position0 instance.
	 */
	private Position0() {
		moveList = new ArrayDeque<Move>();
		unmakeRegisterHistory = new ArrayDeque<PositionStateRecord>();
	 ...
	 */
	private Position0(Position0 pos) {
	...
	/**
	 * Returns a 64 bitboard Zobrist hash key representing the position.
	 * 
	...
	/**
	 * Returns a 64 bitboard Zobrist hash key for the pawn-king structure on the board.
	 * 
	 * @return
	 */
	...
	long getAttackers(int sqrInd, boolean byWhite) {
		...
		if (byWhite) {
			...
			if (enPassantRights != EnPassantRights.NONE.ind && sqrInd == EnPassantRights.TO_W_VICT_SQR_IND + enPassantRights)
				attackers |= whitePawns & dB.kingMoveMask & Rank.R5.bitboard;
		} else {
			...
			if (enPassantRights != EnPassantRights.NONE.ind && sqrInd == EnPassantRights.TO_B_VICT_SQR_IND + enPassantRights)
				attackers |= blackPawns & dB.kingMoveMask & Rank.R4.bitboard;
		}
		return attackers;
	}
	/**
	...
	boolean isAttacked(int sqrInd, boolean byWhite) {
		...
		if (byWhite) {
			return ((whiteKing & dB.kingMoveMask) != 0 ||
					...
					sqrInd == EnPassantRights.TO_W_VICT_SQR_IND + enPassantRights && (whitePawns & dB.kingMoveMask & Rank.R5.bitboard) != 0));
		} else {
			return ((blackKing & dB.kingMoveMask) != 0 ||
					...
					sqrInd == EnPassantRights.TO_B_VICT_SQR_IND + enPassantRights && (blackPawns & dB.kingMoveMask & Rank.R4.bitboard) != 0));
		}
	}
	/**
	...
	long getBlockerCandidates(int sqrInd, boolean byWhite) {
		...
		if (byWhite) {
			...
			if ((sqrBit & Rank.R4.bitboard) != 0 && (allEmpty & blackPawnAdvance) != 0)
			...
			if (enPassantRights == sqrInd%8 && (sqrBit & Rank.R6.bitboard) != 0)
			...
			if ((sqrBit & Rank.R5.bitboard) != 0 && (allEmpty & whitePawnAdvance) != 0)
			...
			if (enPassantRights == sqrInd%8 && (sqrBit & Rank.R3.bitboard) != 0)
	...
	long getCheckers() {
		...
		if (whitesTurn) {
			...
			dB = MoveSetBase.getByIndex(sqrInd);
			attackers = blackKnights & dB.knightMoveMask;
			attackers |= blackPawns & dB.pawnWhiteCaptureMoveMask;
			attackers |= (blackQueens | blackRooks) & dB.getRookMoveSet(allNonWhiteOccupied, allOccupied);
			attackers |= (blackQueens | blackBishops) & dB.getBishopMoveSet(allNonWhiteOccupied, allOccupied);
	...
	boolean isLegalSoft(Move move) {
		...
		if (offsetBoard[move.from] == move.movedPiece) {
			dB = MoveSetBase.getByIndex(move.from);
			PseudoSwitch: {
				if (whitesTurn) {
					if (move.movedPiece == Piece.W_KING.ind) {
						if (move.type == MoveType.SHORT_CASTLING.ind)
							return !inCheck && (whiteCastlingRights == CastlingRights.SHORT.ind || whiteCastlingRights == CastlingRights.ALL.ind) &&
									((Square.F1.bitboard | Square.G1.bitboard) & allOccupied) == 0 && offsetBoard[Square.H1.ind] == Piece.W_ROOK.ind &&
									!isAttacked(Square.F1.ind, false) && !isAttacked(Square.G1.ind, false);
						else if (move.type == MoveType.LONG_CASTLING.ind)
							return !inCheck && (whiteCastlingRights == CastlingRights.LONG.ind || whiteCastlingRights == CastlingRights.ALL.ind) &&
									((Square.B1.bitboard | Square.C1.bitboard | Square.D1.bitboard) & allOccupied) == 0 && offsetBoard[Square.A1.ind] == Piece.W_ROOK.ind &&
					...
					if (move.movedPiece == Piece.B_KING.ind) {
						if (move.type == MoveType.SHORT_CASTLING.ind)
							return !inCheck && (blackCastlingRights == CastlingRights.SHORT.ind || blackCastlingRights == CastlingRights.ALL.ind) &&
									((Square.F8.bitboard | Square.G8.bitboard) & allOccupied) == 0 && offsetBoard[Square.H8.ind] == Piece.B_ROOK.ind &&
									!isAttacked(Square.F8.ind, true) && !isAttacked(Square.G8.ind, true);
						else if (move.type == MoveType.LONG_CASTLING.ind)
							return !inCheck && (blackCastlingRights == CastlingRights.LONG.ind || blackCastlingRights == CastlingRights.ALL.ind) &&
									((Square.B8.bitboard | Square.C8.bitboard | Square.D8.bitboard) & allOccupied) == 0 && offsetBoard[Square.A8.ind] == Piece.B_ROOK.ind &&
			...
			if ((moveSet & toBit) != 0) {
				...
				checked = isAttacked(BitOperations.indexOfBit(whitesTurn ? whiteKing : blackKing), !whitesTurn);
	...
	boolean givesCheck(Move move) {
		...
		makeMoveOnBoard(move);
		givesCheck = isAttacked(BitOperations.indexOfBit(whitesTurn ? blackKing : whiteKing), whitesTurn);
		unmakeMoveOnBoard(move);
		return givesCheck;
	}
	/**
			...
			if ((pinnedPieceBit = BitOperations.getLSBit(attRayMask.filePos & allOccupied) & allWhiteOccupied) != 0) {
				if ((pinnerBit = BitOperations.getLSBit((attRayMask.filePos & allOccupied)^pinnedPieceBit)  & straightSliders) != 0) {
					...
					if (pinnedPiece == Piece.W_QUEEN.ind || pinnedPiece == Piece.W_ROOK.ind) {
						...
						pinnedPieceMoveSet = MoveSetBase.getByIndex(pinnedPieceInd).getWhitePawnAdvanceSet(allEmpty);
						while (pinnedPieceMoveSet != 0) {
							moves.add(new Move(pinnedPieceInd, BitOperations.indexOfLSBit(pinnedPieceMoveSet), pinnedPiece, Piece.NULL.ind,
									MoveType.NORMAL.ind));
							pinnedPieceMoveSet = BitOperations.resetLSBit(pinnedPieceMoveSet);
			...
			if ((pinnedPieceBit = BitOperations.getLSBit(attRayMask.filePos & allOccupied) & allBlackOccupied) != 0) {
				if ((pinnerBit = BitOperations.getLSBit((attRayMask.filePos & allOccupied)^pinnedPieceBit)  & straightSliders) != 0) {
					...
					if (pinnedPiece == Piece.B_QUEEN.ind || pinnedPiece == Piece.B_ROOK.ind) {
						...
						pinnedPieceMoveSet = MoveSetBase.getByIndex(pinnedPieceInd).getBlackPawnAdvanceSet(allEmpty);
						while (pinnedPieceMoveSet != 0) {
							moves.add(new Move(pinnedPieceInd, BitOperations.indexOfLSBit(pinnedPieceMoveSet), pinnedPiece, Piece.NULL.ind,
									MoveType.NORMAL.ind));
							pinnedPieceMoveSet = BitOperations.resetLSBit(pinnedPieceMoveSet);
	...
	private List<Move> generateTacticalMoves() {
		...
		long allNonWhiteOccupiedTemp, allNonBlackOccupiedTemp, allOccupiedTemp;
		...
		if (whitesTurn) {
			...
			moveSet = MoveSetBase.getByIndex(king).getKingMoveSet(allBlackOccupied);
			while (moveSet != 0) {
			...
			while (pieceSet != 0) {
				...
				moveSet = MoveSetBase.getByIndex(piece).getQueenMoveSet(allBlackOccupied, allOccupied);
				while (moveSet != 0) {
					to = BitOperations.indexOfLSBit(moveSet);
			...
			while (pieceSet != 0) {
				piece = BitOperations.indexOfLSBit(pieceSet);
				moveSet	= MoveSetBase.getByIndex(piece).getRookMoveSet(allBlackOccupied, allOccupied);
				while (moveSet != 0) {
					to = BitOperations.indexOfLSBit(moveSet);
			...
			while (pieceSet != 0) {
				piece = BitOperations.indexOfLSBit(pieceSet);
				moveSet	= MoveSetBase.getByIndex(piece).getBishopMoveSet(allBlackOccupied, allOccupied);
				while (moveSet != 0) {
					to = BitOperations.indexOfLSBit(moveSet);
			...
			while (pieceSet != 0) {
				piece = BitOperations.indexOfLSBit(pieceSet);
				moveSet	= MoveSetBase.getByIndex(piece).getKnightMoveSet(allBlackOccupied);
				while (moveSet != 0) {
					to = BitOperations.indexOfLSBit(moveSet);
			...
			while (pieceSet != 0) {
				piece = BitOperations.indexOfLSBit(pieceSet);
				moveSet = MoveSetBase.getByIndex(piece).getWhitePawnMoveSet(allBlackOccupied, allEmpty);
				while (moveSet != 0) {
					to = BitOperations.indexOfLSBit(moveSet);
			...
			if (enPassantRights != EnPassantRights.NONE.ind &&
				...
				while (pieceSet != 0) {
					...
					allNonWhiteOccupiedTemp = allNonWhiteOccupied^enPassAttBits;
					allOccupiedTemp = allOccupied^enPassBits;
					if (((blackQueens | blackRooks) & kingDb.getRookMoveSet(allNonWhiteOccupiedTemp, allOccupiedTemp)) == 0 &&
							((blackQueens | blackBishops) & kingDb.getBishopMoveSet(allNonWhiteOccupiedTemp, allOccupiedTemp)) == 0)
			...
			moveSet	= MoveSetBase.getByIndex(king).getKingMoveSet(allWhiteOccupied);
			while (moveSet != 0) {
				to = BitOperations.indexOfLSBit(moveSet);
			...
			while (pieceSet != 0) {
				piece = BitOperations.indexOfLSBit(pieceSet);
				moveSet	= MoveSetBase.getByIndex(piece).getQueenMoveSet(allWhiteOccupied, allOccupied);
				while (moveSet != 0) {
					to = BitOperations.indexOfLSBit(moveSet);
			...
			while (pieceSet != 0) {
				piece = BitOperations.indexOfLSBit(pieceSet);
				moveSet	= MoveSetBase.getByIndex(piece).getRookMoveSet(allWhiteOccupied, allOccupied);
				while (moveSet != 0) {
					to = BitOperations.indexOfLSBit(moveSet);
			...
			while (pieceSet != 0) {
				piece = BitOperations.indexOfLSBit(pieceSet);
				moveSet	= MoveSetBase.getByIndex(piece).getBishopMoveSet(allWhiteOccupied, allOccupied);
				while (moveSet != 0) {
					to = BitOperations.indexOfLSBit(moveSet);
			...
			while (pieceSet != 0) {
				piece = BitOperations.indexOfLSBit(pieceSet);
				moveSet	= MoveSetBase.getByIndex(piece).getKnightMoveSet(allWhiteOccupied);
				while (moveSet != 0) {
					to = BitOperations.indexOfLSBit(moveSet);
			...
			while (pieceSet != 0) {
				piece = BitOperations.indexOfLSBit(pieceSet);
				moveSet = MoveSetBase.getByIndex(piece).getBlackPawnMoveSet(allWhiteOccupied, allEmpty);
				while (moveSet != 0) {
					to = BitOperations.indexOfLSBit(moveSet);;
					victim = offsetBoard[to];
					// Check for promotion.
			...
			if (enPassantRights != EnPassantRights.NONE.ind &&
				...
				while (pieceSet != 0) {
					...
					enPassAttBits = ((1L << piece) | (1L << to));
					enPassBits = enPassAttBits | (1L << (to + 8));
					allNonBlackOccupiedTemp = allNonBlackOccupied^enPassAttBits;
					allOccupiedTemp = allOccupied^enPassBits;
					if (((whiteQueens | whiteRooks) & kingDb.getRookMoveSet(allNonBlackOccupiedTemp, allOccupiedTemp)) == 0 &&
							((whiteQueens | whiteBishops) & kingDb.getBishopMoveSet(allNonBlackOccupiedTemp, allOccupiedTemp)) == 0)
	...
	private List<Move> generateQuietMoves() {
		...
		if (whitesTurn) {
			king = BitOperations.indexOfBit(whiteKing);
			moveSet = MoveSetBase.getByIndex(king).getKingMoveSet(allEmpty);
			while (moveSet != 0) {
				to = BitOperations.indexOfLSBit(moveSet);
				if (!isAttacked(to, false))
			...
			if ((whiteCastlingRights == CastlingRights.LONG.ind || whiteCastlingRights == CastlingRights.ALL.ind) &&
					((Square.B1.bitboard | Square.C1.bitboard | Square.D1.bitboard) & allOccupied) == 0 && !isAttacked(Square.D1.ind, false) &&
			...
			if ((whiteCastlingRights == CastlingRights.SHORT.ind || whiteCastlingRights == CastlingRights.ALL.ind) &&
					((Square.F1.bitboard | Square.G1.bitboard) & allOccupied) == 0 && !isAttacked(Square.F1.ind, false) && !isAttacked(Square.G1.ind, false))
			...
			while (pieceSet != 0) {
				piece = BitOperations.indexOfLSBit(pieceSet);
				moveSet = MoveSetBase.getByIndex(piece).getQueenMoveSet(allEmpty, allOccupied);
				while (moveSet != 0) {
			...
			while (pieceSet != 0) {
				piece = BitOperations.indexOfLSBit(pieceSet);
				moveSet	= MoveSetBase.getByIndex(piece).getRookMoveSet(allEmpty, allOccupied);
				while (moveSet != 0) {
			...
			while (pieceSet != 0) {
				piece = BitOperations.indexOfLSBit(pieceSet);
				moveSet	= MoveSetBase.getByIndex(piece).getBishopMoveSet(allEmpty, allOccupied);
				while (moveSet != 0) {
			...
			while (pieceSet != 0) {
				piece = BitOperations.indexOfLSBit(pieceSet);
				moveSet	= MoveSetBase.getByIndex(piece).getKnightMoveSet(allEmpty);
				while (moveSet != 0) {
			...
			while (pieceSet != 0) {
				piece = BitOperations.indexOfLSBit(pieceSet);
				moveSet = MoveSetBase.getByIndex(piece).getWhitePawnAdvanceSet(allEmpty);
				while (moveSet != 0) {
					to = BitOperations.indexOfLSBit(moveSet);
					if (to <= 55)
			...
			if ((blackCastlingRights == CastlingRights.SHORT.ind || blackCastlingRights == CastlingRights.ALL.ind) &&
					((Square.F8.bitboard | Square.G8.bitboard) & allOccupied) == 0 && !isAttacked(Square.F8.ind, true) && !isAttacked(Square.G8.ind, true))
			...
			if ((blackCastlingRights == CastlingRights.LONG.ind || blackCastlingRights == CastlingRights.ALL.ind) &&
					((Square.B8.bitboard | Square.C8.bitboard | Square.D8.bitboard) & allOccupied) == 0 && !isAttacked(Square.C8.ind, true) &&
			...
			while (pieceSet != 0) {
				piece = BitOperations.indexOfLSBit(pieceSet);
				moveSet = MoveSetBase.getByIndex(piece).getBlackPawnAdvanceSet(allEmpty);
				while (moveSet != 0) {
					to = BitOperations.indexOfLSBit(moveSet);
					if (to >= 8)
	...
	private List<Move> generateTacticalCheckEvasionMoves() {
		...
		if (whitesTurn) {
			...
			if (BitOperations.resetLSBit(checkers) == 0) {
				...
				dB = MoveSetBase.getByIndex(checker1);
				if ((checkers & Rank.R8.bitboard) != 0)
					...
					if (movedPiece == Piece.W_PAWN.ind) {
						if (promotionOnAttackPossible) {
							...
							moves.add(new Move(checkerAttackerSquare, checker1, movedPiece, checkerPiece1, MoveType.PROMOTION_TO_KNIGHT.ind));
								...
								((1L << checkerAttackerSquare) & ((checkers << 1) | (checkers >>> 1)) & Rank.R5.bitboard) != 0)
				...
				if (checkerPiece1 == Piece.B_QUEEN.ind) {
					if ((File.getBySquareIndex(king).bitboard & checkers) != 0 || (Rank.getBySquareIndex(king).bitboard & checkers) != 0) {
						...
						if (promotionOnAttackPossible && (whiteKing & Rank.R8.bitboard) != 0)
			...
			// Double check, only the king can move.
				...
				dB = MoveSetBase.getByIndex(checker2);
			...
			if (BitOperations.resetLSBit(checkers) == 0) {
				...
				checkerPiece1 = offsetBoard[checker1];
				dB = MoveSetBase.getByIndex(checker1);
				if ((checkers & Rank.R1.bitboard) != 0)
					promotionOnAttackPossible = true;
					...
					if (movedPiece == Piece.B_PAWN.ind) {
						if (promotionOnAttackPossible) {
							...
							moves.add(new Move(checkerAttackerSquare, checker1, movedPiece, checkerPiece1, MoveType.PROMOTION_TO_KNIGHT.ind));
								...
								((1L << checkerAttackerSquare) & ((checkers << 1) | (checkers >>> 1)) & Rank.R4.bitboard) != 0)
							moves.add(new Move(checkerAttackerSquare, (byte) (checker1 - 8), movedPiece,
									checkerPiece1, MoveType.EN_PASSANT.ind));
						else
							moves.add(new Move(checkerAttackerSquare, checker1, movedPiece, checkerPiece1, MoveType.NORMAL.ind));
				...
				if (checkerPiece1 == Piece.W_QUEEN.ind) {
					if ((File.getBySquareIndex(king).bitboard & checkers) != 0 || (Rank.getBySquareIndex(king).bitboard & checkers) != 0) {
						...
						if (promotionOnAttackPossible && (blackKing & Rank.R1.bitboard) != 0)
							promotionOnBlockPossible = true;
					} else
	...
	List<Move> getTacticalMoves() {
		return inCheck? generateTacticalCheckEvasionMoves() : generateTacticalMoves();
	}
	/**
	...
	List<Move> getQuietMoves() {
		return inCheck? generateQuietCheckEvasionMoves() : generateQuietMoves();
	}
	private void makeNormalMoveOnBoard() {

	}
	/**
	 * Makes a move only on the chess board representations of this Position0 object.
	 * 
	 * @param move
	 */
	...
	/**
	 * Unmakes a move only on the chess board representations of this Position0 object.
	 * 
	 * @param move
	 */
	private void unmakeMoveOnBoard(Move move) {
		int enPassantVictimSquare;
		long fromBit = Square.getByIndex(move.from).bitboard;
		long toBit = Square.getByIndex(move.to).bitboard;
		long changedBits = (fromBit | toBit);
		long collChangedBits;
		if (whitesTurn) {
			if (move.type == MoveType.NORMAL.ind) {
	...
	void makeMove(Move move) {
		...
		unmakeRegisterHistory.addFirst(new PositionStateRecord(whiteCastlingRights, blackCastlingRights, enPassantRights, 
	...
	/**
	 * Reverts the changes made to the state of the Position0 instance by the last move made. It returns a reference to the move
	 * unmade.
	 *
	 * @return
	 */
	Move unmakeMove() {
		PositionStateRecord positionInfo = unmakeRegisterHistory.poll();
...
//			return 1;
		...
		try {
			...
			if (san.matches("^O-O[+#]?[//?!]{0,2}$")) {
				...
				if (whitesTurn) {
					movedPiece = Piece.W_PAWN.ind;
					from = BitOperations.indexOfBit(mT.getBlackPawnCaptureSet(whitePawns & movablePieces)
							& File.getByIndex((int) (chars[0] - 'a')).bitboard);
					if (enPassantRights != EnPassantRights.NONE.ind && to == enPassantRights + EnPassantRights.TO_W_DEST_SQR_IND) {
						capturedPiece = Piece.B_PAWN.ind;
						type = MoveType.EN_PASSANT.ind;
					} else {
				...
				to = (byte) ((int) (chars[1] - 'a') + 8*(Integer.parseInt(Character.toString(chars[2])) - 1));
				movedPiece = (byte) (Piece.parse(chars[0]).ind + (whitesTurn ? 0 : Piece.W_PAWN.ind));
				capturedPiece = Piece.NULL.ind;
				type = MoveType.NORMAL.ind;
				from = -1;
				restriction = Bitboard.FULL_BOARD;
				...
				type = MoveType.NORMAL.ind;
				from = -1;
				restriction = File.getByIndex((int) (chars[1] - 'a')).bitboard;
				...
				type = MoveType.NORMAL.ind;
				from = -1;
				restriction = Rank.getByIndex((int) (chars[1] - '1')).bitboard;
				...
				type = MoveType.NORMAL.ind;
				from = -1;
				restriction = File.getByIndex((int) (chars[1] - 'a')).bitboard & Rank.getByIndex((int) (chars[2] - '1')).bitboard;
	...
	String toSAN(Move move) {
		...
		mT = MoveSetBase.getByIndex(move.to);
		movablePieces = 0;
		for (Move m : getMoves())
			movablePieces |= (1L << m.from);
		switch (mPiece) {
		...
		if (origin == null) {
			if (BitOperations.hammingWeight(possOriginSqrs) == 1)
				origin = "";
			else if (BitOperations.hammingWeight(File.getBySquareIndex(move.from).bitboard & possOriginSqrs) == 1)
				origin = Character.toString((char) (move.from%8 + 'a'));
			else if (BitOperations.hammingWeight(Rank.getBySquareIndex(move.from).bitboard & possOriginSqrs) == 1)
