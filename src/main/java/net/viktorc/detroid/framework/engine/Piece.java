...
/**
 * An enum type for the different chess pieces. Each piece has an id number by which it is represented in the array of
 * the auxiliary offset board, and a FEN notation character.
 * 
 * @author Viktor
 *
 */
enum Piece {
	...
	/**
	 * A number that generally represents the respective piece, among others, for example on the offset board.
	 */
	final byte ind;
	/**
	 * A character denoting the piece-type in chess notation.
	 */
	final char letter;
	...
	static Piece parse(char piece) throws ChessParseException {
		switch (piece) {
			...
			default: throw new ChessParseException();
		}
	}
	/**
