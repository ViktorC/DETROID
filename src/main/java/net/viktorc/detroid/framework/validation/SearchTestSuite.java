...
/**
 * An implementation for an EPD test suite for sanity checks and the rough estimation of the tactical strength of a chess engine. Each
 * instance has a name and holds a list of {@link net.viktorc.detroid.framework.validation.EPDRecord} instances.
 *
 * @author Viktor
 */
public class SearchTestSuite {

  public static final String BEST_MOVE_OP_CODE = "bm";
  ...
  public static boolean searchTest(UCIEngine engine, ControllerEngine controllerEngine, EPDRecord record, long timePerPos)
    ...
    for (String san : record.getListOperand(BEST_MOVE_OP_CODE)) {
    ...
    String log = String.format("%s - %s in %.3fs", record, controllerEngine.convertPACNToSAN(bestMove), ((double) duration) / 1000);
