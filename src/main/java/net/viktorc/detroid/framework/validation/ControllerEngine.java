...
public interface ControllerEngine extends UCIEngine {
   ...
   * positions the respective moves lead to as the value set. The moves should be in PACN.
   *
   * @param depth The perft depth. Perft to a depth of this minus one will be executed for each legal move from the current position.
   * @return A set of entries with all legal moves in the current position as the key set and the perft results at depth - 1 for the
  ...
  default Set<Entry<String, Long>> divide(int depth) {
    ...
    for (String m : getLegalMoves()) {
      ...
      unplayLastMove();
      set.add(new SimpleImmutableEntry<>(m, nodes));
    }
    return set;
  }
