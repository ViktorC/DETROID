 ...
 * uses one single thread without having to restart it until a delayed task actually gets executed which allows for
 * increased efficiency. Whenever a new delayed control task needs to be submitted, the timer can be paused with the 
 * {@link #pause() pause} method, the delay, the resolution, and the action to be performed can be reset using the 
 * {@link #setDelay(long) setDelay}, {@link #setResolution(long) setResolution}, and  {@link #setCallBack(Runnable) setCallBack} 
 * methods, and finally the timer can be prompted to continue with the newly set delay, resolution and/or task with the 
 * {@link #goOn() goOn} method. If the timer once went off, it has to be restarted by setting the timing parameters and 
 * calling the {@link #start() start} method which restarts the thread from the single thread fixed thread pool.
 * 
 * @author Viktor
 *
 */
public class SingularTimer implements AutoCloseable {
	...
	// A single thread fixed thread pool increase the timer needs to be restarted.
