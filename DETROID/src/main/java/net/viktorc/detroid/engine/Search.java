...
class Search implements Runnable, Future<SearchResults> {
	...
	private void updateInfo(Position position, Move currentMove, int moveNumber, short ply, int alpha, int beta, int score) {
		...
		// Update stats.
		stats.set(extractPv(position, ply), currentMove, moveNumber, ply, (short) selDepth, adjustedScore.getKey(),
	 ...
	 *
	 */
	class AbnormalSearchTerminationException extends RuntimeException {
